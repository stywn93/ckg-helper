...
def login_and_wait_for_profile(page, username: str, password: str) -> None:
    ...
    try:
        page.wait_for_url(re.compile(r".*/profile(?:[/?#].*)?$"), timeout=LOGIN_SUCCESS_TIMEOUT_MS)
        page.wait_for_load_state("networkidle")
...
def main() -> dict:
    excel_path, sheet_name = resolve_dataset("remaja")
...
if __name__ == "__main__":
    ...
    monitored_main(f"remaja - {username}", main)
